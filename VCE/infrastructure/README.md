...
1) "Cloud Accounts" - Configure connection between vRA8 and exiting vCenter, AWS or Azure deployment. In this example only vCenter is used. 
...
*Important*: The ESXi configuration must have the compute in a "Cluster". If this not the case, then it will fail with an error. Also if one has one ESXi, configuring "Cluster" in ESXi is just a logical configuration and does not force to one to have many ESXi hosts.
